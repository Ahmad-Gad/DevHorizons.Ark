...
namespace DevHorizons.Ark.Dev.TurboCode
{
    ...
    public static class StringDelimiterUnsafe
    {
        ...
        public static ReadOnlySpan<string> SplitLeftUnsafe(this string source, ReadOnlySpan<char> delimiter, StringComparison comparisonType = StringComparison.Ordinal)
        {
            ...
            return source.SplitLeftUnsafe(delimiter, 0, source.Length - 1, comparisonType);
        }
        ...
        public static ReadOnlySpan<string> SplitLeftUnsafe(this string source, ReadOnlySpan<char> delimiter, int start, StringComparison comparisonType = StringComparison.Ordinal)
        {
            ...
            return source.SplitLeftUnsafe(delimiter, start, source.Length - 1, comparisonType);
        }
        ...
        public static ReadOnlySpan<string> SplitLeftUnsafe(this string source, ReadOnlySpan<char> delimiter, int start, int end, StringComparison comparisonType = StringComparison.Ordinal)
        {
            ...
            return SplitLeftUnsafeInternal(source, delimiter, start, end, comparisonType);
        }
        ...
        private unsafe static ReadOnlySpan<string> SplitLeftUnsafeInternal(this ReadOnlySpan<char> source, ReadOnlySpan<char> delimiter, int start, int end, StringComparison comparisonType = StringComparison.Ordinal)
        {
            ...
            fixed(char* pt = source)
            {
                for (var i = start; i <= len; i++)
                {
                    ...
                    {
                        if (delCounter < delimiter.Length)
                        {
                            continue;
                        }
                    }
                    ...
                    if (delimiter.Equals(delSpan, comparisonType))
                    {
                        ...
                        if (markIndex == -1)
                        {
                            ...
                            {
                                spanBank[++bankIndex] = new string(txtCut.SliceLeftSpanInternal(start, i - 1));
                            }
                        }
                        else
                        {
